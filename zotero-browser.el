          ...
          (let* ((node (ewoc-locate zotero-browser-ewoc))
            ...
            (if (equal arg '(4))
              ...
              (let* ((table (zotero-cache-get :type type :id id :resource "items"))
                     (choices (ht-map (lambda (key value) (cons (zotero-lib-plist-get* value :object :data :title) key)) table))
    ...
    (if (zotero-cache-write-access-p library)
        (when (eq major-mode 'zotero-browser-items-mode)
          (let* ((node (ewoc-locate zotero-browser-ewoc))
                 ...
                 (collections (seq-remove (lambda (elt) (equal elt zotero-browser-collection))) collections)
                 (collections (unless (seq-contains-p collections collection)
                                (vconcat (vector collection) collections)))
                 (object (plist-get entry :object))
                 (data (plist-get object :data))
                 (updated-data (plist-put data :collections collections)))
            (zotero-cache-save :type type :id id :resource "items" :data updated-data)
            (zotero-browser-revert)))
      (user-error "Library %s had no write access" id))))
          ...
          (let (nodes
            ...
            (if (use-region-p)
              ...
              (let ((node (ewoc-locate ewoc)))
                (push node nodes)
                (push (ewoc-data node) keys)
                (zotero-cache-restore :type type :id id :resource resource :key (ewoc-data node)))))
          (zotero-browser-revert))
      (user-error "Library %s had no write access" id))))
